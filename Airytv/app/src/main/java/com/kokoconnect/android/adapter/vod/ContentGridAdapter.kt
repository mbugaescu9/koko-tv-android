...
//    }
    ...
    var itemCountToShow: Int = MAX_ITEMS
        set(value) {
            if (field != value) {
    ...
    val dataItems: List<ContentRow>
        get() {
            return items.filter { it is ContentDataGridRow }
        }
    ...
    var spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
        override fun getSpanSize(position: Int): Int {
            return when (items.getOrNull(position)?.rowType) {
    ...
    var gridListItemDecoration = GridSpacingItemDecoration(
    ...
    override fun getItemViewType(position: Int): Int {
        return items.getOrNull(position)?.rowType?.ordinal ?: ContentRowTypes.CONTENT_GRID.ordinal
    }

    override fun getItemCount(): Int {
        return Math.min(items.size, itemCountToShow)
    }
