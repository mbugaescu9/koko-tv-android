...
import com.kokoconnect.android.databinding.FragmentDescriptionFullscreenBinding
...
class DescriptionFullscreenFragment : Fragment(), Injectable {
    @Inject
    ...
    var binding: FragmentDescriptionFullscreenBinding? = null

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentDescriptionFullscreenBinding.inflate(inflater)
        return binding?.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        view?.setOnClickListener {
            setDesctiptionVisible(false)
        }
        tvGuideViewModel.channelNumberLiveData.observe(viewLifecycleOwner, Observer {
            if (it != null) {
                binding?.channelNumber?.text = it.toString()
            }
        })
    }
    ...
    private fun setDesctiptionVisible(visible: Boolean) {
        if (visible) {
    ...
    private fun openPopupDescription() {
        ...
        if (isLandscape && context != null && activity?.contentView?.windowToken != null) {
