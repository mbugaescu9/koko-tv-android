...
public final class ImaAdsLoader implements Player.EventListener, AdsLoader {
  ...
  public static final class Builder {
    ...
    @Nullable private CustomFields customFields;
    ...
    public ImaAdsLoader.Builder setCustomFields(CustomFields fields) {
      this.customFields = Assertions.checkNotNull(fields);
      return this;
    }
  ...
  private ImaAdsLoader(
          ...
          @Nullable CustomFields customFields,
  ...
  public void requestAds(ViewGroup adViewGroup) {
    ...
    Log.d(TAG, "requestAds() uri " + request.getAdTagUrl());
  ...
  private void handleCustomAdErrorEvent(AdErrorEvent adErrorEvent) {
    if (customFields != null) {
      customFields.onAdError(adErrorEvent, this);
    }
  }

  private void handleAdEvent(AdEvent adEvent) {
    if (customFields != null) {
      customFields.onAdEvent(adEvent, this);
    }
  ...
  private int getLoadingAdGroupIndex() {
    ...
    for (AdPlaybackState.AdGroup adGroup : adPlaybackState.adGroups) {
      Log.d(TAG, "getLoadingAdGroupIndex() ad group");
      for (Uri uri : adGroup.uris) {
        Log.d(TAG, "getLoadingAdGroupIndex() ad group uri " + uri);
      }
    }
  ...
  private final class ComponentListener
    ...
    public void onAdError(AdErrorEvent adErrorEvent) {
      ...
      handleCustomAdErrorEvent(adErrorEvent);
      if (DEBUG) {
    ...
    public void loadAd(AdMediaInfo adMediaInfo, AdPodInfo adPodInfo) {
      try {
        ...
        Log.d(TAG, "loadAd() adUri " + adUri);
        for (AdPlaybackState.AdGroup adPlaybackGroup : adPlaybackState.adGroups) {
          Log.d(TAG, "loadAd() ad group");
          for (Uri uri : adPlaybackGroup.uris) {
            Log.d(TAG, "loadAd() ad group uri " + uri);
          }
        }
    ...
    public void playAd(AdMediaInfo adMediaInfo) {
      ...
      Log.d(TAG, "playAd() adUri " + adMediaInfo.getUrl());
      for (AdPlaybackState.AdGroup adPlaybackGroup : adPlaybackState.adGroups) {
        Log.d(TAG, "playAd() ad group");
        for (Uri uri : adPlaybackGroup.uris) {
          Log.d(TAG, "playAd() ad group uri " + uri);
        }
      }
    ...
    private void logAdManager(AdsManager adsManager) {
      if (DEBUG) {
        Ad currentAd = adsManager.getCurrentAd();
        if (currentAd == null) return;
        String adSystem = currentAd.getAdSystem();
        String adId = currentAd.getAdId();
        String advertiserName = currentAd.getAdvertiserName();
        String surveyUrl = currentAd.getSurveyUrl();
